...
// global start filter (user 2026-08-03) — applies to Games/Books/Lab and (2026-09-25) to the Sweep
// tab's view totals, which are re-cut client-side from the pipeline's per-day results.
const SINCE_OPTS = [
...
async function decrypt(buf, password) {
  ...
  // GZIP (2026-09-13): the bundle is now gzipped BEFORE encryption -- it had crossed GitHub's
  // 100 MiB hard file limit, which silently broke every publish for three days. Sniff the gzip
  // magic instead of assuming it, so a bundle built by the older writer still opens and a
  // half-landed publish cannot brick the portal.
  const m = new Uint8Array(pt, 0, 2);
  if (m[0] === 0x1f && m[1] === 0x8b) {
    if (typeof DecompressionStream === "undefined")
      throw new Error("This browser cannot read the compressed bundle — update Chrome/Safari/Firefox.");
    pt = await new Response(new Blob([pt]).stream().pipeThrough(new DecompressionStream("gzip"))).arrayBuffer();
  }
...
async function unlock() {
  ...
  try {
    const buf = await (await dataFetch("bundle.enc?v=fe1a9881bf")).arrayBuffer();
...
// Units staked on a pick. Three states worth distinguishing at a glance, because they mean very
// different things: the admin said nothing (plain 1); he stated a size and we used it (bold);
// he stated one and we did NOT act on it, which is every pick before 2026-09-13 and any pick
// where sizing was disabled or the size was clamped (shown struck-through with the stated value).
function unitsCell(r) {
  const used = r.units != null ? r.units : 1;
  const said = r.units_stated;
  const fmt = u => (u % 1 === 0 ? String(u) : u.toFixed(2).replace(/0$/, ""));
  if (said != null && said !== used)
    return `<span title="the admin stated ${fmt(said)}U; we staked ${fmt(used)}U (sizing off, or clamped by max_units_per_pick)">`
         + `${fmt(used)} <s class="note">${fmt(said)}</s></span>`;
  if (said != null)
    return `<b title="the admin stated this size and we staked it (${r.units_src || "call line"})">${fmt(used)}</b>`;
  // NOT .note (2026-09-14): muted 12px grey in a ten-column table reads as an empty column, which
  // is what prompted "the games tab doesn't show the units". Plain text here, bold above when the
  // admin stated the size, so the column is legible and the distinction still carries.
  return `<span title="no size stated — flat 1 unit">1</span>`;
}
...
function gamesList() {
  ...
  // #91 SIZING. Until 2026-09-13 every entry was a flat 1 unit, so `units` is absent on older
  // rows and must read as 1 -- not 0, which would make the staked total collapse and the
  // per-unit return meaningless on exactly the history we have most of.
  const U = r => (r.units != null ? r.units : 1);
  const staked = closed.reduce((a, r) => a + U(r) * (r.unit || 10), 0);
  const sized = closed.filter(r => r.units != null && r.units !== 1).length;
  const perU = closed.length ? tot / closed.reduce((a, r) => a + U(r), 0) : 0;
  // ALWAYS shown (2026-09-14, user: "the games tab doesn't show the units"). This was gated on
  // `sized` -- i.e. it only appeared once some pick was staked at other than 1 unit. Sizing has
  // been live since 2026-09-13 and every pick so far is still 1.0U, so the gate was never true
  // and the whole line silently evaluated to "". Staked and per-unit are meaningful at a flat
  // 1U too, and "sized 0" is itself the answer to "has sizing ever actually fired?".
  const sizeLine = ` · <span title="Picks where the admin STATED a size and we staked it (#91). Everything else is a flat 1 unit — 0 here means sizing is live but has not yet changed a stake.">sized <b>${sized}</b></span>`
    + ` · <span title="Total staked across closed picks, at the unit size actually used.">staked $${staked.toFixed(0)}</span>`
    + ` · <span title="Paper P&L divided by units staked — comparable across picks of different size, unlike raw P&L.">per unit <span class="${perU >= 0 ? "good" : "bad"}">${perU.toFixed(2)}</span></span>`;
  ...
  view().innerHTML = liveWarn + `<h3>Games (${rows.length}) <small>· pick won ${pct(pw, decided)} ·
    profitable ${pct(prof, closed)} · <span title="PAPER book total — the simulation's ledger, not the account">paper P&L <span class="${tot >= 0 ? "good" : "bad"}">${tot.toFixed(2)}</span></span>${sizeLine}${liveLine}${lastTxt}</small></h3>` + unmatchedNote + `<div class="scrollbox"><table class="list" id="gameslist"><thead><tr>
    <th>date</th><th>txn</th><th>player</th><th title="Units staked. 1 unless the admin stated a size and sizing was enabled (#91).">u</th><th>entry</th><th>exit</th><th>outcome</th><th>P&L</th>
    ...
    `</tbody></table></div>` + altMarketPanel() + cloudPanel();
...
// Cloud decisions (#97h): what the cloud layer was asked and what came of it. Counts by CLOUD_*
// state over the export's window, then the last decisions. Fed by DATA.cloud, which older bundles
// do not carry -- the panel is simply absent then. Nothing here touches the paper totals.
function cloudPanel() {
  const cl = DATA.cloud;
  if (!cl || (!(cl.n > 0) && !((cl.decisions || []).length))) return "";
  const TIP = { CLOUD_HANDOFF: "local path abstained -> cloud consulted", CLOUD_DONE: "cloud acted",
    CLOUD_REFUSED: "cloud asked for an action the executor refused", CLOUD_CAPPED: "clipped by the risk caps",
    CLOUD_SKIPPED: "skipped: disabled, low confidence, or already taken locally", CLOUD_STALE: "decision arrived too late",
    CLOUD_DUP: "re-decided an id already decided" };
  const states = Object.entries(cl.states || {}).sort((a, b) => b[1] - a[1])
    .map(([s, n]) => `<span title="${attr(TIP[s] || s)}"><b>${n}</b> ${s.replace("CLOUD_", "").toLowerCase()}</span>`).join(" · ");
  const rows = (cl.decisions || []).slice().reverse();
  const conf = c => c === "certain" ? `<b class="good">${c}</b>` : c === "unsure" ? `<span class="note">${c}</span>` : (c || "");
  return `<h3 style="margin-top:1.4em">Cloud decisions <small>(last ${cl.days || 14}d: ${cl.n || 0} events · ${states || "none"})</small></h3>` +
    (rows.length ? `<div class="scrollbox"><table class="list"><thead><tr>
    <th>when</th><th>action</th><th>market</th><th>player</th><th>size</th><th>confidence</th><th>outcome</th><th>reason</th></tr></thead><tbody>` +
    rows.map(r => `<tr class="${r.discarded ? "rowdead" : ""}">
      <td>${r.t ? fmtPT(r.t).slice(0, 16) : ""}</td><td>${r.kind && r.kind !== "decision" && !r.action ? `<span class="note">${r.kind}</span>` : (r.action || "")}</td>
      <td><small>${r.market || ""}</small></td><td>${r.player || ""}</td>
      <td>${r.units != null ? `${r.units}U` : r.fraction != null ? `${Math.round(r.fraction * 100)}%` : ""}</td>
      <td>${conf(r.confidence)}</td>
      <td class="${r.discarded ? "bad" : r.ok === false ? "bad" : ""}">${r.discarded ? `discarded: ${r.discarded}` : r.ok === false ? "error" : (r.action && r.action !== "none" ? "queued" : "")}</td>
      <td><small>${attr(r.reason || "")}</small></td></tr>`).join("") +
    `</tbody></table></div>` : `<p class="note">no decisions recorded yet</p>`);
}
...
function gameView(k) {
  ...
  view().innerHTML = `${nav}<h3>${r.player} — ${r.mkt || "?"} <small>(${r.outcome || "active"}, P&L ${r.pnl ?? "?"}${stakeTitle(r)})</small></h3>
...
// MODULE SCOPE, not inside gameView (fixed 2026-09-21). These helpers were inserted between
// gameView's nav template and its render, so they were scoped to gameView -- and gamesList's
// cloud badge (isCloud/positionLegs, #97k) plus computeBooks' positionSize threw
// "isCloud is not defined" on every load. The Games and Books tabs rendered nothing from
// 2026-09-16 until now; Sweep and Lab never touched them, which is why they kept working.
...
// The stake ACTUALLY used on this pick, for the chart header and the rules line. Before #91 every
// entry was one flat unit, so an absent `units` means 1 -- never 0.
function stakeUnits(r) { return r.units != null ? r.units : 1; }

function stakeTitle(r) {
  const u = stakeUnits(r);
  return u === 1 ? "" : `, ${u}U`;
}

// #97h multi-leg. The legs of a position AS FILLED (n0; `n` is what is still open and reads 0
// after the close), the contract-weighted entry basis, and the dollars actually staked across
// them. Rows without `legs` return an empty list and every caller falls back to the old fields.
function positionLegs(r) {
  return Array.isArray(r.legs) ? r.legs.filter(l => l && ((l.n0 ?? l.n) || 0) > 0 && l.ask != null) : [];
}
function positionSize(r) {
  const legs = positionLegs(r);
  if (!legs.length) { const e = r.entry_price; return { C: e ? 10 / (e / 100) : 0, e, legs: 0, staked: null }; }
  const C = legs.reduce((a, l) => a + (l.n0 ?? l.n), 0);
  const e = legs.reduce((a, l) => a + (l.n0 ?? l.n) * l.ask, 0) / C;
  return { C, e, legs: legs.length, staked: C * e / 100 };
}
const isCloud = r => r.origin === "cloud" || positionLegs(r).some(l => l.origin === "cloud")
  || (Array.isArray(r.partials) && r.partials.some(q => q.closed_by === "cloud"));

function stakeCell(r, R) {
  const u = stakeUnits(r), said = r.units_stated, base = R.unit;
  const P = positionSize(r);
  if (P.legs > 1)          // scaled position: what was actually staked across the legs
    return `<b title="${P.legs} legs (${positionLegs(r).map(l => `${l.n0 ?? l.n} ct @ ${l.ask}¢${l.origin === "cloud" ? " cloud" : ""}`).join(", ")}); basis ${P.e.toFixed(1)}¢${said != null ? `; the admin stated ${said}U` : ""}">`
         + `stake $${P.staked.toFixed(0)} across ${P.legs} legs (${u}U)</b>`;
  if (said != null && said !== u)
    return `<span title="the admin stated ${said}U; this pick staked ${u}U (sizing off at the time, or clamped)">`
         + `stake $${(base * u).toFixed(0)} (${u}U, said <s>${said}U</s>)</span>`;
  if (said != null)
    return `<b class="good" title="the admin stated this size (${r.units_src || "call line"}) and it was staked">`
         + `stake $${(base * u).toFixed(0)} (${u}U)</b>`;
  return `unit $${base}`;
}
...
function collectMarks(r, scores, t0, ours) {
  ...
  // #97h multi-leg: one ENTRY marker per leg (a local entry plus cloud scale-ins), each at its
  // own sample and price. A single-leg row keeps the original label; rows without `legs` (every
  // lifecycle before #97h) take the old entry_idx path unchanged.
  const legs = positionLegs(r);
  if (legs.length > 1) {
    legs.forEach((l, i) => {
      const t = l.idx != null ? t0 + (r.toff?.[l.idx] ?? l.idx * (r.freq_s || 5)) : (l.ts ? Date.parse(l.ts) / 1000 : null);
      add(t, `ENTRY ${i + 1}/${legs.length} · ${l.n0 ?? l.n} ct @ ${l.ask}¢${l.origin === "cloud" ? " · cloud" : ""}`,
          l.origin === "cloud" ? "#1abc9c" : "#2ecc71", i ? `entry+${i + 1}` : "entry");
    });
  } else if (r.entry_idx != null) add(t0 + (r.toff?.[r.entry_idx] ?? r.entry_idx * (r.freq_s || 5)), `ENTRY @ ${r.entry_price}¢${legs.length && legs[0].origin === "cloud" ? " · cloud" : ""}`, "#2ecc71", "entry");
  ...
  // #97h partial exits (cloud scale-outs) before the final EXIT, each at its own sample/price
  for (const q of (Array.isArray(r.partials) ? r.partials : []))
    add(q.idx != null ? t0 + (r.toff?.[q.idx] ?? 0) : (q.ts ? Date.parse(q.ts) / 1000 : null),
        `PARTIAL ${q.reason || ""} · ${q.n} ct @ ${q.px}¢${q.closed_by === "cloud" ? " · cloud" : ""}${q.pnl != null ? ` (${q.pnl >= 0 ? "+" : ""}${q.pnl})` : ""}`,
        "#e67e22", "partial");
...
function computeBooks(P = {}) {
  ...
  for (const r of done) {
    ...
    // #97h: a multi-leg position's size is Σ leg contracts at the contract-weighted basis;
    // without legs this is the old 10/(e/100) at entry_price, unchanged.
    const PS = positionSize(r);
    const aEff = effTaker(PS.e);
    ...
    for (const [k, opt] of Object.entries(pols)) {
      ...
      if (res.outcome === "skip" || res.outcome === "maker-miss" || !ev) { books[k].skips++; }
        ...
        const pC = PS.legs ? PS.C : Math.round(10 / (ev.px / 100));
    ...
    const e = PS.e, C = PS.legs ? PS.C : (e ? 10 / (e / 100) : 0);
...
// ---------------- Sweep ----------------
// 2026-09-25: the research pipeline's nightly grid (research/reinterp, nightly-v1) replaces the old
// classifier/sweep.py text grids; numbers and HTML come from sweeps.js (pure, jsc-checked), this
// function binds events. The header's start selector re-cuts the view totals client-side.
let SWEEP_F = { filters: {}, sort: "view" };
function sweepView() {
  const sw = DATA.sweeps;
  if (!SW.isValid(sw)) {
    view().innerHTML = `<h3>Strategy sweep</h3><p class="note">No pipeline sweep in this bundle${
      sw && sw.why ? ` (${String(sw.why).replace(/</g, "&lt;")})` : ""}. The tab fills after the next
      successful ops/run_sweep_pipeline.sh run.</p>`;
    return;
  }
  const rows = SW.viewRows(sw, SINCE);
  const wf = SW.foldRanks(sw);
  const legacy = DATA.frozen50 ? `<details class="sw-sec"><summary><b>Legacy: frozen top-50 watch</b> (old classifier/sweep.py engine, locked ${DATA.frozen50.frozen_at}; not comparable with the table above)</summary>
  ...
  view().innerHTML = SW.headerHTML(sw, rows, SINCE) + `<div id="margins"></div>` +
    SW.filterBarHTML(sw, SWEEP_F.filters, SWEEP_F.sort) +
    `<p class="note">★ = the fixed pre-registered choice, pinned on top. Right-click a row to take it to the What-if lab. Sept* is exploratory${SW.hasForward(sw) ? "; fwd* is exploratory for every config except the ★ row, whose forward number is the clean out-of-sample test" : ""}.</p>
    <div class="scrollbox"><table class="list" id="grid"></table></div>` + legacy;
  const render = () => {
    const rs = SW.applyFilters(rows, SWEEP_F.filters, SWEEP_F.sort);
    document.getElementById("fcount").textContent = `${rs.length} shown`;
    document.getElementById("grid").innerHTML = SW.tableHTML(sw, rs, wf);
    document.getElementById("margins").innerHTML = SW.marginalsHTML(sw, rs);
  };
  ...
  document.getElementById("grid").addEventListener("contextmenu", e => {
    ...
    m.querySelector('[data-act="lab"]').onclick = () => {
      window.__LABPRESET = SW.labPreset(tr.dataset);
      m.style.display = "none";
      location.hash = "#lab";
    };
    m.querySelector('[data-act="filter"]').onclick = () => {
      const d = tr.dataset;
      SWEEP_F.filters = { ceiling: d.c, tp: d.tp, stop: d.stop, cfm: d.cfm, follow: d.follow };
      m.style.display = "none";
      view().querySelectorAll("[data-f]").forEach(x => { x.value = SWEEP_F.filters[x.dataset.f] ?? ""; });
      render();
    };
  });
  view().querySelectorAll(".fbar select").forEach(e => e.addEventListener("change", () => {
    view().querySelectorAll("[data-f]").forEach(x => { SWEEP_F.filters[x.dataset.f] = x.value; });
    SWEEP_F.sort = document.getElementById("s").value;
    render();
  }));
  document.getElementById("freset").addEventListener("click", () => {
    SWEEP_F = { filters: {}, sort: "view" };
    view().querySelectorAll("[data-f]").forEach(e => e.value = "");
    document.getElementById("s").value = "view";
    render();
  });
  render();
}
